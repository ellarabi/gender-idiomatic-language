	...
	with codecs.open(filename, 'r', 'utf-8') as fin:
		...
		for line in fin:
			for word in line.lower().split():
				...
				#if word in stopwords or not word.isalpha(): continue
				current = result.get(word, 0)
...
def main():
	...
	counts1 = load_counts(args.first, args.min_count, stopwords)
	counts2 = load_counts(args.second, args.min_count, stopwords)
	...
	for word, log_odds in sorted(delta.items(), key=lambda x: x[1]):
		# skip non-alphabetical other than contractions
		#if not word.isalpha() and "'" not in word: continue
